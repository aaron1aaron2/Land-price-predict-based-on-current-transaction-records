...
for gp_id in tqdm.tqdm(df_tar['group_id'].to_list()):
    ...
    for col in target_cols:
        ...
        df_coor_dup = df_coor_dup.assign(**{f'{col}_DIST':list(map(get_distance, df_coor_dup[[transaction_coordinate_col, col]].values))})
        df_coor_dup = df_coor_dup[~(df_coor_dup[f'{col}_DIST']=='')]

    df_coor_dup.to_csv(os.path.join(output_folder, f'group{gp_id}_distance.csv'), index=False)