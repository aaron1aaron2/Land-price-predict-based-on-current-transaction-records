...
def get_args():
    ...
    parser.add_argument('--t', type=str, default='output/test-hyper-parameter(amount)')
...
if __name__ == '__main__':
    ...
    eval_ls = glob.glob(os.path.join(args.t, '**', '*evaluation.json'), recursive=True)
    ...
    tmp = result['file'].str.replace(args.t, '', regex=False)
    ...
    if args.output:
        ...
        result.to_csv(os.path.join(args.t, 'result.csv'), index=False)
