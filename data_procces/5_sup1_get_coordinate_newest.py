...
Describe: 獲取土地經緯度座標，來源 「地籍圖資網路便民服務系統」。最新資料-> https://easymap.land.moi.gov.tw/
            ...
            因爬取速度較慢，所以本方案排在 2_get_coordinate.py 之後，處理無法獲取的最新資料
-----------------------------------
Issue: 
...
def get_args():
    ...
    parser.add_argument('--file_path', type=str, default='data/procces/5_get_coordinate(target)/crawler_miss.csv')
    parser.add_argument('--output_folder', type=str, default='data/procces/5_sup1_get_coordinate(target)')
...
def get_new_land_code(df:pd.DataFrame, section_dt:dict, output:str, county_col:str, district_col:str, section_col:str, number_col:str) -> None:
    ...
    if os.path.exists(result_path):
        tmp = df[county_col] + df[district_col] + df[section_col] + df[number_col]
        ...
        df = df[~tmp.isin(all_df[county_col] + all_df[district_col] + all_df[section_col] + all_df[number_col])]
    ...
    for i in tqdm(land_addr_dt_ls):
        ...
        while main_ct < 3:
            ...
            try:
                ...
                city = [ls_txt.text for ls_txt in city_options if ls_txt.text.find(i[county_col]) != -1]
                ...
                while len(town) == 0:
                    ...
                    town = [ls_txt.text for ls_txt in town_options if ls_txt.text.find(i[district_col]) != -1] 
                ...
                while len(section) == 0:
                    ...
                    section = [val for ls_txt, val in zip(section_options, section_options_val) if ls_txt.find(i[section_col]) != -1] 
                ...
                while ct < 10:
                    ...
                    landcode_tar.send_keys(i[number_col])
                ...
                # 儲存查詢資訊
                if (result_text.find('地號') != -1) | ("lat" in coor_dt):

                    i.update({
                        'lat':coor_dt["lat"] if 'lat' in coor_dt else '',
                        'long': coor_dt["lon"] if 'lon' in coor_dt else '',
...
def main():
    ...
    # 爬取座標資料
    get_new_land_code(df, section_dt, args.output_folder, args.county_col, args.district_col, args.section_col, args.number_col)
